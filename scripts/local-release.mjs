...
const packages = [
	{ directory: "packages/ai", name: "@theatechcorporation/pi-ai" },
	{ directory: "packages/tui", name: "@theatechcorporation/pi-tui" },
	{ directory: "packages/agent", name: "@theatechcorporation/pi-agent-core" },
	{ directory: "packages/coding-agent", name: "@theatechcorporation/pi-coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "a-coder-cli-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `a-coder-cli-${platform}.zip` : `a-coder-cli-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "a-coder-cli.cmd"))) {
			writeFileSync(join(installDirectory, "a-coder-cli.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\a-coder-cli.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "a-coder-cli.ps1"), '& "$PSScriptRoot/node_modules/.bin/a-coder-cli.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "a-coder-cli.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\a-coder-cli.exe" %*\r\n');
		writeFileSync(join(installDirectory, "a-coder-cli.ps1"), '& "$PSScriptRoot/node_modules/.bin/a-coder-cli.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "a-coder-cli"), join(installDirectory, "a-coder-cli"));
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `a-coder-cli-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "a-coder-cli.exe" : "a-coder-cli")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "a-coder-cli.cmd" : "a-coder-cli")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "a-coder-cli.cmd" : "a-coder-cli")} --help`);
	}
}
