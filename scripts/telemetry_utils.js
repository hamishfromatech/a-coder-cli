...
const USER_GEMINI_DIR = path.join(os.homedir(), '.a-coder');
// Project-level .gemini directory in the workspace
const WORKSPACE_GEMINI_DIR = path.join(projectRoot, '.a-coder');
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `a-coder-cli-releases-${Date.now()}.json`,
  );
  try {
    execSync(
      `curl -sL -H "User-Agent: a-coder-cli-dev-script" -o "${tmpFile}" "${url}"`,
      { stdio: 'pipe' },
    );
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'a-coder-cli-telemetry-'),
  );
