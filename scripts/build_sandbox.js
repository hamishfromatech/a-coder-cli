...
console.log('packing @a-coder/cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'a-coder-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @a-coder/cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @a-coder/core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'a-coder-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @a-coder/core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `a-coder-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `a-coder-core-${packageVersion}.tgz`),
  0o755,
);
