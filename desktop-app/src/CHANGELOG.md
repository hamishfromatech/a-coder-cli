...
- Added a **Mobile access** settings section: start/stop the `a-coder-cli serve` bridge for the current workspace (new tauri commands `serve_start`/`serve_stop`/`serve_status` stream the CLI's stderr banner as `desktop://serve-line` events), with the pairing QR rendered in-app from the banner's endpoint + token, a copyable manual token, live bridge log tail, and status chip. The CLI owns token persistence; the desktop just relays the banner.
- First-launch CLI bootstrap is now fully automatic: when the desktop can't find the CLI engine, it starts downloading and installing the matching release immediately (no click), with progress in the boot overlay; the app reloads when done. The install button remains as a fallback if the automatic install fails. Version-sync on later launches was already automatic and is unchanged.
...
- The thinking card's copy affordance now lives inside the card header (appears on card hover, copies the thinking content) instead of the reply-copy button floating over the card's corner: the reply-copy button anchors to the reply text block, so each copy control belongs to the content it copies.
- Session tabs now reliably carry the session's title. Two bugs fed the strip stale names: `syncEngineState` only applied the engine name when truthy, so switching from a named session to an unnamed one made the new tab inherit the previous session's title; and the tab-sync effect ran on the optimistic switch path before `get_state` returned, renaming the freshly activated tab with the PREVIOUS session's stale name. The name is now always applied (including clearing), and the tab relabel only happens once the name is authoritative for that session — the store keeps an existing tab's label while the switch is still in flight.
- Full media and file-type support in the Files panel viewer: PDFs render natively in a sandboxed iframe, CSV/TSV files preview as a sticky-header table (capped at 300 rows / 32 columns), and images stream in AVIF/HEIC/TIFF/JFIF alongside PNG/JPEG/WebP/GIF/BMP/ICO/SVG. All binary media (images, audio, video, PDF) now stream through Tauri's asset protocol instead of base64-over-IPC — no more 8MB preview cap, any file size, and video seeking works via Range requests. The code/text map grew substantially (Swift, Kotlin, Java, C/C++/C#, Objective-C, Dart, Ruby, PHP, Lua, Perl, R, Julia, Elixir, Erlang, Haskell, Elm, Clojure, F#, Zig, Nim, Groovy, Vue, Svelte, Astro, GraphQL, Prisma, HCL/Terraform, Protobuf, Solidity, Nix, batch/PowerShell, LaTeX, RST, AsciiDoc, diff/patch, CMake, Makefile, and more). WebM now classifies as video (it previously landed in audio). Players surface a friendly "format can't be played in-app" fallback with an open-with-system-player escape hatch for containers WebKit can't decode (MKV, AVI, WMV, FLV).
- Persistent session rail in the left sidebar: every stored session with a status dot (blocked-on-input amber, turn running accent, finished-while-away green, idle grey), readable titles (user-set name → first user message), recency grouping (Today / Yesterday / Previous 7 days / Earlier), type-to-filter, and a New-session button. Clicking a row resumes via the same engine switch path as the tab strip; cross-project sessions show their workspace leaf. Sections now read Sessions → Projects → Current session → Tree, so "what sessions exist and what are they doing" is answered at a glance instead of only behind the ⌘P picker.

- Virtual office floor view in the Office panel: a live 2D/3D "digital twin" of the office (building-icon toggle in the panel header, 2D/3D segmented switch in the scene). The 2D floor renders zone-tinted floors (planked workstations, carpeted meeting pods, lounge, corridor), dressed furniture with shadows, walk-animated coworker avatars built from the engine's Face model (status rings, thinking dots, tool chips, speech bubbles, spawn ring), curved collaboration links between coworkers who talk to each other, and meeting-pod gathering while a huddle drive runs. The 3D scene (three.js + @react-three/fiber, lazy-loaded on first switch so the bundle stays lean) adds walled rooms with emissive windows, shadow-casting furniture, capsule characters with walk bob / work bounce / speak bounce, status auras, canvas-texture name labels, and a drag-to-orbit camera. Fed by the engine's new `office_activity` events plus roster/huddle state (`virtual-office-ui/` folder, `src/components/office/FloorView.tsx`).
- Your Office panel (right sidebar, third tab next to Files/Git): the roster of named coworkers with procedural faces (deterministic color + shape from the handle, or a custom face), huddles, and errands. Huddles render as chat threads with avatar bubbles, a live "…is working" indicator with a Stop button, per-coworker supervised prompt cards (approvals/questions), and a composer with @handle chips; DMs open from a coworker row. Editors cover hiring/editing coworkers (name, role, mission, soul, face palette + shape, model override, supervised/auto autonomy), seating huddles (max 6), and scheduling errands (every/daily/once, continuity, DM or huddle delivery, run-now). The panel is fed by the engine's `office_update`/`office_huddle` events and talks through the new `office_*` RPC commands (`src/components/office/`, `stores/office-store.ts`, typed wrappers in `lib/rpc.ts`).
- Full-screen, frameless app window: the native OS titlebar is gone. On macOS the window uses Tauri's overlay titlebar style, so the native traffic lights float over the app content (positioned to sit vertically centered in the in-app titlebar, hidden window title); on Windows/Linux the window is undecorated with custom Windows-style minimize/maximize/restore/close controls flush at the titlebar's right edge (`WindowControls.tsx`, close hover is red). The in-app titlebar is now the real titlebar: it carries `data-tauri-drag-region` (drag from anywhere in the bar, double-click toggles maximize), reserves space for the traffic lights on macOS, and keeps its existing controls. Window-control and drag permissions were already present in `src-tauri/capabilities/default.json`. Configuration split: `tauri.conf.json` sets `decorations: false` for Windows/Linux, and the new `tauri.macos.conf.json` (platform-merged by the Tauri CLI) overrides the main window with `titleBarStyle: "Overlay"` + `hiddenTitle` + `trafficLightPosition` — the windows array is fully repeated there because platform config merge replaces arrays.

- Office panel turn-end chimes: when a coworker's turn finishes naturally (huddle or DM), the app plays the same completion sound the main composer uses (same enable toggle, variant and volume from Settings — Look & feel), plus the stream-done haptic. Composer stop-reason rules apply: aborted turns (office Stop), in-session errors and retryable failures stay silent. Simultaneous completions in a huddle drive are throttled to one cue per 700ms so a round of finishing coworkers doesn't stack into a wall of sound.
- Session-loading screen: while a session switch or resume loads its history (engine `session_start`, or a `--continue` launch after the CONNECTING overlay), the chat area shows a loading state instead of the new-conversation EmptyState — a lemniscate-bloom loader with a whimsical verb, the session's name/file, and shimmering skeleton rows — so the blank pause between loading sessions is never mistaken for an empty conversation. The flag clears as soon as history paints (or the load fails), and the warm-cache fast path (previously visited sessions) still paints instantly.
...
- Tool approvals never use a full-screen modal: permission requests and generic extension confirms always render as the inline approval bar in the transcript (with the floating fallback above the composer when scrolled away) — full-screen blurred modal backdrops could black out the entire window on macOS WKWebView when mounted mid-stream. Select/input/editor requests keep the modal.
- Office panel header rebranded to "A-Coder Virtual Office" to match the web version.
...
- Fixed the desktop auto-updater failing with "Update failed. Check your internet connection and try again." on every platform. Two pipeline bugs: the `latest.json` updater manifest referenced the bundler's original asset names, but GitHub normalizes release asset names (spaces become dots), so the macOS download URL 404'd (`A-Coder Desktop.app.tar.gz` vs the stored `A-Coder.Desktop.app.tar.gz`); and the Windows/Linux entries were never generated at all because the manifest generator hunted Tauri v1 updater artifacts (`*-setup.nsis.zip`, `*.AppImage.tar.gz`) that Tauri v2 never produces — v2 ships the raw `-setup.exe` and `.AppImage` next to their `.sig` files. The generator now maps v2 artifacts (`windows-x86_64`, `linux-x86_64`), emits GitHub-normalized asset names in URLs, and the staging step copies `*.AppImage.sig`. The updater endpoint points at the renamed `a-coder-cli` repo instead of relying on the `pi-mono` rename redirect. The v0.80.62 release manifest was patched in place (darwin URL fix + Windows entry; Linux stays without auto-update for this one release since its `.AppImage.sig` was never staged). The update error modal now surfaces the updater's actual error message below the generic advice instead of swallowing it.
...
- Added a connection status dot to the titlebar, consistent with the status bar.
