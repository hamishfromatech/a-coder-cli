...
// Fourteen curated presets, picked in Settings → Look & feel. Default is variant 1.
//
// Ported from Hermes' desktop completion-sound lib. a-coder desktop is a single
// window, so the cross-window dedupe (ownsAmbientCue) Hermes needed is dropped —
// playCompletionSound() just plays. Enable/variant state lives in the local
// settings store (useSettingsStore) rather than nanostores atoms.
...
// Range mirrors COMPLETION_SOUND_VARIANTS below. Validating by range (not
// membership) keeps this free of a dependency on the variants array — and the
// store, which reads this constant, stays free of a dependency on this lib
// (which imports the store back). A membership check would close that cycle.
export const COMPLETION_SOUND_VARIANT_COUNT = 14;

export function resolveCompletionSoundVariantId(variantId: number): number {
	return Number.isInteger(variantId) && variantId >= 1 && variantId <= COMPLETION_SOUND_VARIANT_COUNT
		? variantId
		: DEFAULT_COMPLETION_SOUND_VARIANT_ID;
}

let ctx: AudioContext | null = null;
let firstGesturePrimeInstalled = false;
...
/**
 * Create + resume the completion-sound `AudioContext` within a user gesture.
 * WKWebView's autoplay policy only resumes a suspended `AudioContext` from
 * inside an active gesture, and `playCompletionSound()` fires async on
 * `agent_end` (no gesture on the stack). Calling this on the first
 * pointer/keydown primes the context so the first turn-end chime isn't muted.
 * Safe to call repeatedly; only the first call that actually resumes matters.
 */
...
/**
 * Attach one-time gesture listeners that prime the audio context on the
 * first user interaction, then self-remove. Call once at app boot.
 */
export function installFirstGestureAudioPrime(): void {
	if (typeof window === "undefined" || firstGesturePrimeInstalled) {
		return;
	}
	firstGesturePrimeInstalled = true;
...
// Audition a variant from settings. Bypasses the enable toggle so the sound can
...
// enable toggle in the settings store.
export function playCompletionSound() {
	const { completionSoundEnabled, completionSoundVariantId } = useSettingsStore.getState();
