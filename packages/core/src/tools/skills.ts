...
export class SkillsTool extends BaseTool<SkillsToolParams, ToolResult> {
  ...
  constructor() {
      ...
      'Manage and load specialized skills to enhance capabilities. Use "list" to see available skills and "load" to activate a skill.',
      {
        properties: {
          action: {
            type: Type.STRING,
            enum: ['list', 'load'],
            description: 'The action to perform: "list" to show available skills, "load" to activate a skill.',
          },
          skill_name: {
            type: Type.STRING,
            description: 'The name of the skill to load (required if action is "load").',
          },
        },
        required: ['action'],
        type: Type.OBJECT,
    ...
    this.skillsDir = path.join(os.homedir(), GEMINI_DIR, 'skills');
  }
    ...
    if (params.action === 'list') {
        try {
            ...
            const skills = entries
                ...
                .filter(dirent => fs.existsSync(path.join(this.skillsDir, dirent.name, 'SKILL.md')))
        ...
        if (!fs.existsSync(skillPath)) {
            // Check if user meant a directory that exists but has no SKILL.md
             if (fs.existsSync(path.join(this.skillsDir, skillName)) && fs.statSync(path.join(this.skillsDir, skillName)).isDirectory()) {
                  return {
