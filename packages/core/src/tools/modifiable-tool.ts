...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'a-coder-cli-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `a-coder-cli-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `a-coder-cli-modify-${fileName}-new-${timestamp}${ext}`,
  );
