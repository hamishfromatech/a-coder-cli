...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".a-coder-cli", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".a-coder-cli", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".a-coder-cli", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".a-coder-cli"), { recursive: true });
			writeFileSync(join(cwd, ".a-coder-cli", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".a-coder-cli"), { recursive: true, force: true });
