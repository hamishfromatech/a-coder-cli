...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.A_CODER_CLI_TELEMETRY;
		delete process.env.A_CODER_CLI_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.A_CODER_CLI_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://a-coder-cli.dev");
		expect(headers?.["X-OpenRouter-Title"]).toBe("a-coder-cli");
	...
	it("adds default attribution headers for Vercel AI Gateway models", async () => {
		...
		expect(headers?.["http-referer"]).toBe("https://a-coder-cli.dev");
		expect(headers?.["x-title"]).toBe("a-coder-cli");
	});
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe("a-coder-cli");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("a-coder-cli");
	});
