...
describe("strict built-in tools", () => {
	...
	it("allows extensions to re-register tools without strict sampling", async () => {
		...
		try {
			...
			try {
				...
				// The fork rebuilds the system prompt from the active tool set; the
				// re-registered (non-strict) read tool must still be present.
				expect(session.getToolDefinition("read")?.promptSnippet).toContain("Read file contents");
