...
describe("package commands", () => {
	...
	beforeEach(() => {
		...
		originalPiPackageDir = process.env.A_CODER_CLI_PACKAGE_DIR;
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".a-coder-cli"), { recursive: true });
		writeFileSync(join(projectDir, ".a-coder-cli", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".a-coder-cli", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".a-coder-cli", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("a-coder-cli install <source> [-l]");
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: a-coder-cli install <source> [-l]");
...
`,
		...
		process.env.A_CODER_CLI_PACKAGE_DIR = selfPackageDir;
		...
		try {
			...
			expect(stdout).toContain(`Updated a-coder-cli from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated a-coder-cli from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain(`Updated a-coder-cli`);
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		...
		try {
			...
			expect(stdout).not.toContain("Updated a-coder-cli-formatter");
			expect(process.exitCode).toBe(1);
