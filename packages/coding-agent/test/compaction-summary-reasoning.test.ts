...
import type { AssistantMessage, Model } from "@theatechcorporation/pi-ai";
...
vi.mock("@theatechcorporation/pi-ai/compat", async (importOriginal) => {
	const actual = await importOriginal<typeof import("@theatechcorporation/pi-ai/compat")>();
	return {
		...actual,
...
describe("generateSummary reasoning options", () => {
	...
	it("sets Anthropic refusal fallback from model metadata", async () => {
		...
		expect(completeSimpleMock).toHaveBeenCalledTimes(1);
		expect(completeSimpleMock.mock.calls[0][2]).toHaveProperty("refusalFallbacks", [{ model: "claude-opus-4-8" }]);
	});
	...
	it("clamps compaction summary maxTokens to the model output cap", async () => {
		const preparation: CompactionPreparation = {
			...
			settings: { enabled: true, reserveTokens: 500000, keepRecentTokens: 20000, autoContinue: false },
		};
