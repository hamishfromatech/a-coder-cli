...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".a-coder-cli", "prompts");
			...
			const projectSkillDir = join(cwd, ".a-coder-cli", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".a-coder-cli", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".a-coder-cli", "themes"), { recursive: true });
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".a-coder-cli"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".a-coder-cli", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".a-coder-cli", "extensions", "shared.ts"));
		...
		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			...
			const projectExtDir = join(cwd, ".a-coder-cli", "extensions");
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.a-coder-cli", async () => {
			const piDir = join(cwd, ".a-coder-cli");
