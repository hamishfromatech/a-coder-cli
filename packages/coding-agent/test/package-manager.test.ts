...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.A_CODER_CLI_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.A_CODER_CLI_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .a-coder-cli skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .a-coder-cli", async () => {
			const extDir = join(tempDir, ".a-coder-cli", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".a-coder-cli"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".a-coder-cli", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".a-coder-cli", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".a-coder-cli", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".a-coder-cli", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".a-coder-cli", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .a-coder-cli/agent skills", async () => {
		...
		it("should use the project .a-coder-cli dir as baseDir for project .a-coder-cli skills", async () => {
			const projectBaseDir = join(tempDir, ".a-coder-cli");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".a-coder-cli", "agent");
		...
		it("should dedupe user skill entries when ~/.a-coder-cli/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .a-coder-cli auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".a-coder-cli\n");

			const skillDir = join(tempDir, ".a-coder-cli", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.a-coder-cli\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".a-coder-cli", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .a-coder-cli settings base", () => {
			...
			const rel = relative(join(tempDir, ".a-coder-cli"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".a-coder-cli", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".a-coder-cli", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".a-coder-cli", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".a-coder-cli", "npm", "node_modules", "example");
