...
export function getSelfUpdateUnavailableInstruction(
	...
	if (method === "bun-binary") {
		return `Download from: https://github.com/hamishfromatech/pi-mono/releases/latest`;
	}
...
export function getPackageDir(): string {
	...
	const envDir = process.env.A_CODER_CLI_PACKAGE_DIR;
	if (envDir) {
...
const DEFAULT_SHARE_VIEWER_URL = "https://a-coder-cli.dev/session/";

/** Get the share viewer URL for a gist ID */
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = process.env.A_CODER_CLI_SHARE_VIEWER_URL || DEFAULT_SHARE_VIEWER_URL;
