import type { Api, Model, ProviderHeaders } from "@theatechcorporation/pi-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://a-coder-cli.dev",
			"X-OpenRouter-Title": "a-coder-cli",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "a-coder-cli",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "a-coder-cli" };
}
