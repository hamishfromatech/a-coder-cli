<p align="center">
  <a href="https://github.com/hamishfromatech/a-coder-cli">
    <img alt="A-Coder CLI" src="../website/assets/logo-trans.png" width="128">
  </a>
</p>
<p align="center">
  <a href="https://discord.com/invite/3cU7Bz4UPx"><img alt="Discord" src="https://img.shields.io/badge/discord-community-5865F2?style=flat-square&logo=discord&logoColor=white" /></a>
  <a href="https://github.com/hamishfromatech/a-coder-cli/releases"><img alt="releases" src="https://img.shields.io/github/v/tag/hamishfromatech/a-coder-cli?style=flat-square" /></a>
</p>
...
**Custom providers & models:** Add providers via `~/.a-coder/cli/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.a-coder/cli/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
A-Coder CLI has three separate startup features:

- **Update check:** queries the repo's GitHub releases API (`api.github.com/repos/hamishfromatech/a-coder-cli/releases/latest`, override with `A_CODER_LATEST_VERSION_URL`) to check whether a newer version exists. Disable it with `A_CODER_CLI_SKIP_VERSION_CHECK=1`. Disabling update checks only turns off this check.
- **Install telemetry:** gates optional provider attribution headers for OpenRouter, Cloudflare, and direct NVIDIA NIM requests. Opt out by setting `enableInstallTelemetry` to `false` in `settings.json`, or by setting `A_CODER_CLI_TELEMETRY=0`. This does not disable update checks; the CLI may still contact the GitHub API for the latest version unless update checks are disabled or offline mode is enabled.
...
- `~/.a-coder/cli/agent/AGENTS.md` (global)
...
```markdown
<!-- ~/.a-coder/cli/agent/prompts/review.md -->
...
```markdown
<!-- ~/.a-coder/cli/agent/skills/my-skill/SKILL.md -->
# My Skill
...
Packages install to `~/.a-coder/cli/agent/git/` (git) or `~/.a-coder/cli/agent/npm/` (npm). Use `-l` for project-local installs (`.pi/git/`, `.pi/npm/`). Git `@ref` values are pinned tags or commits; pinned packages are skipped by `a-coder-cli update --extensions` and `a-coder-cli update --all`, so use `a-coder-cli install git:host/user/repo@new-ref` to move an existing package to a new ref. Git packages install dependencies with `npm install --omit=dev` by default, so runtime deps must be listed under `dependencies`; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers. If you use a Node version manager and want package installs to reuse a stable npm context, set `npmCommand` in `settings.json`, for example `["mise", "exec", "node@20", "--", "npm"]`.
...
|----------|-------------|
| `A_CODER_CLI_CODING_AGENT_DIR` | Override config directory (default: `~/.a-coder/cli/agent`) |
...
- [@earendil-works/pi-ai](packages/ai): Core LLM toolkit
- [@earendil-works/pi-agent-core](packages/agent): Agent framework
- [@earendil-works/pi-tui](packages/tui): Terminal UI components

<p align="center">
  <a href="https://github.com/hamishfromatech/a-coder-cli">github.com/hamishfromatech/a-coder-cli</a>
</p>
