> a-coder-cli can help you use the SDK. Ask it to build an integration for your use case.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.a-coder-cli/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.a-coder-cli/extensions/`)
- Project skills:
  - `.a-coder-cli/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.a-coder-cli/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.a-coder-cli/agent/skills/`)
  - `~/.agents/skills/`
