...
| `enableAnalytics` | boolean | `false` | Opt-in anonymous usage analytics via PostHog: session lifecycle events with aggregate counters (mode, model, token/cost totals). Never includes prompt content, file paths, or personal information. Opt in during first-time setup or by setting the value directly |
...
For VS Code, include `--wait` so a-coder-cli resumes after the editor exits:
...
| `compaction.autoContinue` | boolean | `false` | Automatically continue after auto-compaction |
...
```json
{ "sessionDir": ".a-coder-cli/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `A_CODER_CLI_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
