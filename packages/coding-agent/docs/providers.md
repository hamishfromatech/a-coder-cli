...
Pi supports subscription-based providers via OAuth and API key providers via environment variables or auth file. For each provider, a-coder-cli knows all available models. The list is updated with every a-coder-cli release.
...
Use `/logout` to clear credentials. Tokens are stored in `~/.a-coder/cli/agent/auth.json` and auto-refresh when expired.
...
Store credentials in `~/.a-coder/cli/agent/auth.json`:
...
Use this when a-coder-cli should use different provider settings than the project shell environment.
...
```bash
a-coder-cli --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
a-coder-cli --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
a-coder-cli --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
For normal a-coder-cli usage, prefer unified billing or stored BYOK. Inline BYOK requires configuring an additional upstream `Authorization` header for the Cloudflare AI Gateway provider, for example via a `models.json` provider/model override.
