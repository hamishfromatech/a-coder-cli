...
```bash
a-coder-cli --mode json "Your prompt"
```

Outputs all session events as JSON lines to stdout. Useful for integrating a-coder-cli into other tools or custom UIs.
...
```bash
a-coder-cli --mode json "List files" 2>/dev/null | jq -c 'select(.type == "message_end")'
```
