> A-Coder CLI can create extensions. Ask it to build one for your use case.
...
```typescript
import type { ExtensionAPI } from "@theatechcorporation/pi-coding-agent";
...
Extensions are auto-discovered from trusted locations. Project-local `.a-coder-cli/extensions` entries load only after the project is trusted.
...
| Package | Purpose |
|---------|---------|
| `@theatechcorporation/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@theatechcorporation/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@theatechcorporation/pi-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed pi packages installed with `a-coder-cli install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
If the factory returns a `Promise`, a-coder-cli awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `pi.registerProvider()` are flushed.
...
This pattern makes the fetched models available during normal startup and to `a-coder-cli --list-models`.
...
Fired before a-coder-cli decides whether to trust a project with dynamic configs (`.pi` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether a-coder-cli asks, trusts, or declines by default.
...
After a successful switch or new-session action, a-coder-cli emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork or clone, a-coder-cli emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
Before `tool_call` runs, a-coder-cli waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@theatechcorporation/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@theatechcorporation/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@theatechcorporation/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@theatechcorporation/pi-coding-agent";
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while a-coder-cli is idle.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `a-coder-cli` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@theatechcorporation/pi-coding-agent";
...
- Captured old `a-coder-cli` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
If multiple extensions register the same command name, a-coder-cli keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@theatechcorporation/pi-tui";
...
```typescript
import { Box, Text } from "@theatechcorporation/pi-tui";
...
If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. a-coder-cli waits for the factory before startup continues, so the registered models are available immediately, including to `a-coder-cli --list-models`.
...
```typescript
import { withFileMutationQueue } from "@theatechcorporation/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@theatechcorporation/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when a-coder-cli resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
a-coder-cli --no-builtin-tools -e ./my-extension.ts
```
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@theatechcorporation/pi-coding-agent";
...
```typescript
import { createBashTool } from "@theatechcorporation/pi-coding-agent";
...
```typescript
import { keyHint } from "@theatechcorporation/pi-coding-agent";
...
```typescript
import { Text, Component } from "@theatechcorporation/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@theatechcorporation/pi-coding-agent";
import { matchesKey } from "@theatechcorporation/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@theatechcorporation/pi-coding-agent";
