...
A-Coder CLI packages bundle extensions, skills, prompt templates, and themes so you can share them through npm or git. A package can declare resources in `package.json` under the `a-coder-cli` key, or use conventional directories.
...
```bash
a-coder-cli install npm:@foo/bar@1.0.0
a-coder-cli install git:github.com/user/repo@v1
a-coder-cli install https://github.com/user/repo  # raw URLs work too
a-coder-cli install /absolute/path/to/package
a-coder-cli install ./relative/path/to/package

a-coder-cli remove npm:@foo/bar
a-coder-cli list                     # show installed packages from settings
a-coder-cli update                   # update a-coder-cli only
a-coder-cli update --all             # update a-coder-cli, update packages, and reconcile pinned git refs
a-coder-cli update --extensions      # update packages and reconcile pinned git refs only
a-coder-cli update --self            # update a-coder-cli only
a-coder-cli update --self --force    # reinstall a-coder-cli even if current
a-coder-cli update npm:@foo/bar      # update one package
...
```bash
a-coder-cli -e npm:@foo/bar
a-coder-cli -e git:github.com/user/repo
```

## Package Sources

A-Coder CLI accepts three source types in settings and `a-coder-cli install`.
...
# With version ref
a-coder-cli install git:git@github.com:user/repo@v1.0.0
```
...
Local paths point to files or directories on disk and are added to settings without copying. Relative paths are resolved against the settings file they appear in. If the path is a file, it loads as a single extension. If it is a directory, a-coder-cli loads resources using package rules.

## Creating a Package

Add a `a-coder-cli` manifest to `package.json` or use conventional directories. Include the `pi-package` keyword for discoverability.
...
The [package gallery](https://a-coder-cli.dev/packages) displays packages tagged with `pi-package`. Add `video` or `image` fields to show a preview:
...
If no `a-coder-cli` manifest is present, a-coder-cli auto-discovers resources from these directories:
