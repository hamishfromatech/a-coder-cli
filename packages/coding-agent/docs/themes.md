> a-coder-cli can create themes. Ask it to build one for your setup.
...
On first run, a-coder-cli detects your terminal background and defaults to `dark` or `light`.
...
**Hot reload:** When you edit the currently active custom theme file, a-coder-cli reloads it automatically for immediate visual feedback.
...
Pi uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, a-coder-cli falls back to the nearest approximation.
