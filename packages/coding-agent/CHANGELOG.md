...
- Added declarative SOP workflows (dynamic multi-agent runs, upstream Claude Code dynamic-workflows parity): a workflow is a `.sop.md` file whose frontmatter declares a machine-readable `workflow.steps` array — ordered `run` steps, `fan-out` steps iterating a previous step's array output via `over`, and loop semantics via `until` / `max_rounds` / `stop_on_no_progress` — that the new `run_workflow` tool executes over in-process background subagents. Every transformation is itself an agent step (deliberately no scripting): prompts interpolate `${item}`, `${stepId.path}`, and `${args.x}`, structured outputs are validated against per-step JSON Schemas with retries, provable schema self-contradictions fail statically before any agent spawns, concurrency is capped by the new `workflowMaxConcurrentAgents` setting (1-256, default 16), runs are bounded by a 1000-agent cap and a 50-minute budget, and a large-run advisory fires past 25 projected agents. Run state persists under the session directory; `run_workflow { resume: "<runId>" }` replays a run reusing saved step results whose rendered inputs are unchanged (refusing while its agents are live), and `/workflows` lists runs, drills into step results and live agents, and stops running runs. Active runs render a live per-step progress widget below the editor (mirrored on the desktop's extension-widget surface), launch flows through normal permission evaluation, and the `workflowKeywordTrigger` setting (default on) transforms a typed prompt containing "ultracode" so the model authors + runs a workflow for the task instead of working turn by turn. A process-level run registry drives host progress surfaces: RPC clients receive live `workflows_update` summaries (output-free, per-step status) and can stop runs with the `stop_workflow_run` command. Ships a bundled `deep-research` example (search fan-out → read → adversarial verify → cited report), a `workflow-authoring` skill, and format docs in `docs/sops.md`.
