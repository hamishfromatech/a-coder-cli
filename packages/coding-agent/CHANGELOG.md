...
- Experimental `computer` tool, configurable via the new `computerUse` setting (default off; the `A_CODER_CLI_COMPUTER_USE=1` env still overrides): desktop control on macOS/Windows/Linux driven through the external **cua-driver** binary (from the cua repo) over MCP-on-stdio, mirroring the transport contract Hermes uses against the same driver. One tool, `action` discriminator: `capture` (som = screenshot with numbered element overlays + AX element list, vision = plain screenshot, ax = tree only), `click`/`double_click`/`right_click`/`middle_click`, `drag`, `scroll`, `type`, `key`, `set_value`, `wait`, `list_apps`, `list_windows`, `focus_app`. Element-index clicking with snapshot `element_token` staleness detection; background-first input delivery (no focus steal), `delivery_mode: "foreground"` gated on the driver's live schema (refused, never silently downgraded); sticky-target guard returns `input_target_mismatch` instead of typing into the wrong window; every result carries a verdict (done / verify fresh state / escalate). Destructive system shortcuts (logout/lock/empty-trash, alias- and hyphen-proof) and dangerous `type` payloads (pipe-to-shell, `sudo rm -rf`, fork bombs) are hard-blocked before approval. Permission-aware: read actions (capture/list/wait) are free; input actions weigh like mutating tools in every mode (blocked in read-only, prompt in ask, classifier-checked in auto). Fail-closed recovery: a timed-out driver call's outcome is unknown — never replayed, transport rebuilt before the next call. Driver binary resolution: `A_CODER_CUA_DRIVER_CMD` override, then PATH; child env strips provider API keys and disables cua-driver telemetry by default. The TUI settings screen gates enabling behind a disclaimer submenu explaining how desktop control works (labeled screenshots, no focus steal, permission-mode gating, hard blocks) with an explicit "I understand — enable" choice; the toggle persists via settings and takes effect when a session rebuilds its tool runtime.
- RPC engines announce themselves: the first stdout frame in `--mode rpc` is an `engine_info` event carrying the engine version and a protocol contract number (`RPC_CONTRACT` in rpc-types.ts, currently 1), so embedded clients can detect engine/client skew early instead of failing cryptically downstream. Clients predating the event ignore it and treat the engine as contract 1. The serve bridge's `server_version` handshake frame and `/info` endpoint now carry a matching `contract` field.
- Auto-update handoff marker: the TUI's self-update writes `update-handoff.json` before relaunching, and the next startup reports the outcome — "updated to vX" when the relaunch landed on the new version, or "an update to vX did not complete" with a retry hint when it didn't. The auto-update also no longer tears down the TUI mid-compaction (previously only mid-response was guarded).
- Serve bridge: client lines the engine cannot accept (not started, exited, or crash-restarting) are refused back to the sender with a new `engine_unavailable` bridge control frame instead of being dropped silently, so an approval answered during an engine crash can no longer vanish while the agent blocks on it forever. A stdin `error` handler also keeps a write racing the engine's death (EPIPE) from taking down the whole serve process.
...
- Mobile bridge: new `a-coder-cli serve` subcommand exposes the same RPC engine protocol to mobile clients over an authenticated WebSocket (`/rpc`; Bearer header or `?token=`). The bridge spawns the engine (`--mode rpc`, same contract as the desktop app) lazily on the first authenticated client and forwards NDJSON frames verbatim in both directions, injecting only `"type":"bridge"` control frames (`connected`, `engine_exited`, `engine_failed`, `shutting_down`, `server_version`). Pairing: a per-machine 256-bit token (timing-safe compare, persisted under `~/.a-coder-cli/agent/serve/token`, `--rotate` to regenerate) is printed to the terminal as a QR code plus 4×8 manual chunks; discovery via mDNS (`_a-coder._tcp`, `--no-announce` to disable); failed handshakes are rate-limited per IP (10 per 15 min); slow clients are dropped above an 8 MiB buffer; crash-restart policy max 2 restarts; default idle-exit after 30 clientless minutes (`--idle-exit 0` disables). Flags: `--cwd`, `--port` (8787 with auto-probe), `--bind`, `--provider`/`--model`, `--continue`, `--token`, `--max-clients`, `--idle-exit`, `--no-auth` (loopback only), `--no-announce`, `--rotate`.
- The serve bridge now sends a per-client `server_version` control frame immediately after the WebSocket handshake completes (clients treat it as auth-complete and start their initial sync; matches the mobile app's pairing protocol).
- `set_session_name` now accepts an optional `sessionPath` so clients can rename background sessions without switching to them; without it, the active session is renamed as before.

- Desktop onboarding tour. First launch now plays a ~2-minute guided walkthrough video (chat and sessions, the model picker, approvals, workflows/ultracode, Your Office, scheduled tasks, the activity inbox, voice mode, and CLI engine parity) before the folder picker; it autoplays muted with burned-in captions and can be skipped. Settings > General gains a "Welcome tour" control that replays it anytime; if the bundled video is missing the modal falls straight back to the folder picker.
...
- Added the `queue_remove` RPC command (`{ kind: "steering" | "followUp", index }`) to remove a single queued message by index, with `AgentSession.removeQueuedMessage` and per-index steering/follow-up removal in the agent queue — clients (the desktop queued-messages panel) can dequeue individual items instead of only clearing the whole queue via `clear_queue`.

- Added script workflows (dynamic multi-agent runs, upstream Claude Code dynamic-workflows parity): a workflow is a plain JavaScript file — `export const meta = { name, description }` plus a top-level-await body — that orchestrates in-process background subagents through runtime primitives injected into a sandboxed context: `agent(prompt, {schema, label, model, agentType})` (structured outputs validated against JSON Schemas with retries; proven schema self-contradictions fail statically; stops/failures resolve to null), `pipeline(list, fn)` (one agent per item, null slots preserved), `parallel(tasks)`, `phase(title)`, `log(msg)`, and the `args` global. The script holds the loop, branching, filtering, and intermediate results — the model's context only sees the final answer. Determinism guards (`Date.now()`, `Math.random()`, no-argument `new Date()` throw) make runs replayable: run state persists a start-ordered agent log under the session directory, and `run_workflow { resume: "<runId>" }` re-executes the script — completed agents whose prompts are unchanged replay from cache, everything from the first changed prompt onward reruns (refusing while its agents are live). Concurrency is capped by the `workflowMaxConcurrentAgents` setting (1-256, default 16) via a run-wide semaphore; runs are bounded by a 4096-item cap per pipeline/parallel call, a 1000-agent cap, and a 50-minute budget, with a large-run advisory past 25 agents. Saved workflows register as `/<name>` slash commands (project `.a-coder-cli/workflows/*.js` wins over the personal dir, which wins over package-provided `workflows/*.js` distributed via the packages system) and launch through normal permission evaluation — arg-scoped rules pre-approve by name (`run_workflow(<name>)`) or for all (`run_workflow(*)`); `/workflows` lists runs, drills into per-agent status/tokens and live sub-agents, stops runs or single agents, restarts a running agent, pauses/resumes, and saves a run's script as a command. Rate-limit-looking agent failures auto-pause the run (status paused, completed agents kept) and retry the same request on resume, up to two waits. A prompt-cache stagger holds same-signature fan-out siblings until the leader's response begins (`workflowPrefixStaggerMs`, default 5000, 0 disables). A `/ultracode` command toggles session-scoped auto-orchestration: every typed prompt carries guidance to plan substantive tasks as dynamic workflows while trivial tasks stay inline. Workflow meta blocks can declare `phases: ['...']` (deduped, validated) so progress and approval surfaces show phase groups before any phase() call runs. The desktop renders run_workflow permission prompts as a workflow approval card — workflow name, declared phase chips, and a token-usage caution — with Allow once / Always allow (grants a session-scoped `run_workflow(<name>)` rule via the new `add_session_allow_rules` RPC command) / Deny actions. A process-level run registry drives host progress surfaces: RPC clients receive live `workflows_update` summaries (output-free, per-phase and per-agent status) and can stop runs with the `stop_workflow_run` command. Active runs render a live per-phase progress widget below the editor (mirrored on the desktop's extension-widget surface), and the `workflowKeywordTrigger` setting (default on) transforms a typed prompt containing "ultracode" so the model authors + runs a workflow for the task instead of working turn by turn. Ships a bundled `deep-research` example (search fan-out → read → adversarial verify → cited report), a `workflow-authoring` skill, and format docs in `docs/sops.md`.
- Added the SOP authoring system for skills (`.sop.md`): ports the Strands agent-sop conventions as parameterized, constraint-based skill workflows — named parameters collected upfront, per-step RFC 2119 constraint blocks (MUST/SHOULD/MAY), explicit interactive-vs-auto mode behavior, and `progress.md` resumability. SOPs are ordinary Agent Skills with structural validation on load (warnings only) via the new `sop.ts` module wired into skill loading; format spec in `docs/sops.md`, template + worked example under `examples/sops/`, and an `sop-author` authoring skill.
- Added team handoff guardrails and shared secret redaction: `send_message` feeds a rolling-window handoff guard (8-message window, 3 unique sender→recipient pairs) that appends an advisory when the exchange collapses into a loop, `spawn_subagent` refuses to clobber a running subagent id, and shared redaction primitives (`PROTECTED_ENV_VARS` blocklist, sensitive-key masking, `redactRecord`) apply the two-tier doctrine that no consent bypass disables redaction — analytics capture redacts unconditionally.
- Bench works from anywhere: the 5 starter tasks are embedded in the package (`bench init` or the wizard scaffolds them into `./bench` when no `bench/tasks` exists nearby; existing files are never overwritten), with a `Scaffold starter tasks` action in the desktop Bench panel for projects without tasks.
- Added A-Coder Bench (`a-coder bench`): an interactive benchmark wizard for measuring (model × harness) pairs — model selection (searchable catalog with local providers discovered live), runs per task, task selection, a how-it-works briefing, and a health & safety confirmation (tool access in allow mode, sandbox-is-working-directory-not-security-boundary, real-token cost warning) before any model is invoked. Runs execute headless in fresh per-run repo sandboxes with wall-clock timeouts and hidden graders (the grader is never visible to the agent), record raw event streams plus structured results to `bench/results.jsonl`, and finish with a rendered leaderboard (pass@1, pass@any, tokens, turns, tool/edit-failure rates) written to `bench/leaderboard.md`. Self-hosted endpoints (vLLM, SGLang, LM Studio, ollama serve) plug in via `--endpoint`/`--api`/`--api-key` through a per-run `models.json` custom provider; `--model provider/id` and `--runs N` skip wizard steps. The shared execution core lives in `packages/coding-agent/src/bench/`; a standalone `bench/runner.ts` script drives the same core for scripted runs. Ships with 5 starter tasks (crash fix, implement-function with hidden edge cases, off-by-one with a fully hidden suite, precise JSON edit, refactor-extract) whose graders are verified to fail on broken repos and pass on golden solutions.

- Added A-Coder Cloud (Sovereign Edition) v0, the self-hosted always-on agent fleet built on Open-PC's architecture: new `@earendil-works/pi-cloud` package with a task store, git sync layer (`ac-cloud/<task-id>` branches, WIP checkpoint commits, diff summaries, optional origin push), a daemon (`a-coder cloud serve`, unix-socket IPC, auto-started on demand, orphaned tasks re-queued after daemon restart), and a task runner that drives an a-coder RPC agent in a cloned workspace — checkpointing, wall-clock budget, extension-UI prompts auto-declined for headless runs. Tasks can be driven with a saved declarative workflow (`--workflow <name> [--args <json>]`, prompt optional) that the worker executes via run_workflow; finalize summarizes the workflow runs into the record and copies their state into the task artifacts for continuity. The agent writes `report.md` into the repo itself; the control plane adds a structured report (commits, diffstat, changed files, token/tool usage, warnings, workflow runs) plus session continuity (session JSONL copied into the task artifacts). CLI: `a-coder cloud spawn <repo> [prompt...] [--workflow --args --base --model --timeout --push]`, `status [--json]`, `review <id>` (prints workflow runs and failed steps), `stop <id>`, `serve`.
...
- Added the `pi auth` command surface (upstream pi 99e34013d/a261366bd parity): `auth print-api-key`, `auth print-bearer-token --min-expiry <duration>`, and `auth check [--json] [--credentials] [--no-refresh]` resolve credentials through the model registry and auth storage without starting a session. `--no-refresh` only refreshes tokens that are already expired; `--min-expiry` requires validity for the requested window and refreshes proactively.
...
- Fixed local Ollama (and LM Studio / llama.cpp) models erroring with "No API key found" even though keyless local servers need no key: the `/model` picker listed these models as available (keyless), but model selection, prompt submission, and the streaming auth resolver still required stored credentials — only users who had gone through `/login` (which stores a `not-needed` placeholder) could actually use them. Keyless local providers are now treated as configured everywhere: `hasConfiguredAuth` exempts them (matching `getAvailable`), and request-time auth supplies the same no-key placeholder credential `/login` stores when nothing else is configured. Real stored keys (auth.json or models.json) still take precedence.
...
- Session picker 1-9 quick keys (easy-agent SessionPicker parity): with an empty search field, pressing 1-9 jumps straight to that row and resumes it.
- `statusLine` setting (easy-agent useStatusLine parity): set `statusLine` in settings.json to a shell command and its first stdout line renders as an extra row above the footer. The command runs on a 300ms trailing debounce on session events with a 2s timeout, receiving a JSON context on stdin (`model`, `cwd`, `permissionMode`, `contextPercent`, `tokens`) — Claude-Code-style custom footers without writing an extension.
- The MCP client now negotiates the `roots` capability and reports the session's working directory to `roots/list` (root defaults to cwd; resource-loader passes the session cwd). Roots-aware servers like chrome-devtools-mcp scope their file-writing tools to the project instead of falling back to the OS temp directory, so `take_screenshot` with a `filePath` inside the session's project works; paths outside stay denied.
- `/context` visualizes the context window split (System prompt / Tool definitions / Conversation history / Free space) as proportional bars with token counts and a low-space hint (easy-agent/Claude Code parity).
- `/diff [n]` shows the git working-tree status, shortstat and a budgeted colored patch plus the file-history section: what rewinding the last n user turns would change (easy-agent /diff parity).
- Background-task notifications render as compact status pills instead of raw multi-line notes in the user bubble (easy-agent extractTaskNotification parity): a completed/failed/killed note collapses to one colored line (glyph + command/subagent summary + duration); the wake turn renders pills only, with the boilerplate instruction line suppressed from display. The model still receives the full note text.
- Historical thinking folds away (easy-agent hidePastThinking parity): only the latest round's thinking trace renders — earlier assistant messages' thinking is hidden from the live transcript so scrolling reads as an activity stream. The verbose transcript (Ctrl+Shift+T) temporarily unfolds everything, and the global hide-thinking setting still takes precedence.
- `ask_user_question` gains an implicit "Other" row (desktop AskUserQuestionCard parity): an extra choice after the model's options that opens a free-text input — type a message and press Enter to send it as the answer (single-select advances, multi-select adds it to the staged selection), Esc returns to the options. The tool description now tells the model the UI provides Other automatically.
- Nested slash-command suggestions (easy-agent commandPalette parity): typing `/office ` suggests the verbs the command consumes (hire/huddle/tell/say/stop/errands) with usage descriptions, and `/export ` suggests the output formats; both filter as you type.
- Tool cards show a "waiting for approval" phase while their permission prompt is open (easy-agent toolStatusStore parity): the session emits `tool_permission_update` events around the user prompt, and the pending card renders a warning badge until the call executes.
- Exit-plan approval dialog (easy-agent PlanApprovalDialog parity): when the model leaves plan mode, an interactive dialog previews the plan file and offers "Yes, proceed", "Yes, switch to ask mode" (exit + permission mode switches to ask so every edit still prompts), or "No, keep planning" with typed feedback that is returned to the model while plan mode stays on. Headless/RPC sessions without the handler exit directly, as before.
- The running-tasks viewer's agent timeline is fully scrollable (easy-agent TeammateViewer parity): ↑/↓ page back through the complete event history (3 events per tick) with a hidden-events count and a "(↓ returns to live)" affordance, instead of a fixed 12-event tail.
- Office `turn_end` activity events now carry the coworker turn's stop reason and a willRetry flag (composer stop-reason parity), so clients can chime on natural turn ends only — aborts (office Stop), in-session errors and retryable failures stay quiet.
- Office activity events for visualizations: while a coworker's turn runs, the engine pushes `office_activity` RPC events (turn lifecycle, tool start/end with tool name, completed assistant speech — never streaming deltas) so clients can render live office scenes; failed turns surface as a system line in the huddle log with the reason instead of reading as a silent pass.
- Your Office: a society of named coworkers with faces, huddles (group conversations), and errands (scheduled jobs with memory) living inside the engine — a team, not a toolbox. Coworkers are persisted under `~/.a-coder/cli/office/` (roster, huddle logs, errands) and each keeps its own canonical session per project, so its memory and history survive restarts; the soul (persona + collaboration protocol) is pinned into that session at first contact. A user send into a huddle triggers a bounded round-robin drive (max 3 rounds, max 10 messages): who speaks each round is a deterministic @mention parse, every coworker is fed only the room delta since its last turn, replying exactly "(pass)" is silence, and a round where everyone passed settles the room. User sends carry hold directives ("stop @atlas" holds a coworker until re-addressed); turns that outlive their window are harvested late instead of lost. Supervised coworkers route tool approvals and questions to the user as office prompts (`office_respond` answers them); auto coworkers run with the auto permission mode. Errands fire on a 30s ticker with every/daily/once schedules and, with continuity, run inside the coworker's own session so scheduled agents actually learn between runs. Surfaces: new `office_*` RPC commands + `office_update`/`office_huddle` events (the desktop Office panel — roster, huddle chat, coworker/huddle/errand editors, procedural faces — is built on them, third tab in the right sidebar) and a `/office` TUI command (`/office`, `hire`, `huddle`, `tell`, `say`, `stop`, `errands`). Coworker side runtimes are created via the new `AgentSessionRuntime.createSideRuntime`, outside the active/detached registries so they never count against background-session caps.
- Opt-in product analytics via PostHog: when `enableAnalytics` is on in settings.json (asked during the experimental first-time setup; the desktop Settings panel also exposes the toggle), the CLI captures anonymous `cli_session_start` / `cli_session_end` events (version, mode, OS/arch, model/provider, aggregate token/cost/tool-call counters; never prompt content, paths, or personal data) with the `trackingId` UUID as distinct ID, plus `cli_model_usage` — one event per (provider, serving model) pair actually used in the session, with per-model turns/tokens/cost, so mid-session model switches are counted accurately (`requested_model` records the picked model when the provider serves a different concrete one, e.g. OpenRouter auto-routing) — and a `cli_install` event alongside the existing install ping. Events fire in interactive, print, RPC (desktop engine), and ACP modes; RPC end-of-session is captured on engine shutdown. Gate overrides: `A_CODER_CLI_ANALYTICS=1/0`, offline mode always disables. Ingest key is baked into `core/analytics.ts` with `A_CODER_CLI_POSTHOG_KEY`/`A_CODER_CLI_POSTHOG_HOST` overrides for self-hosting/testing (an explicitly empty key disables the client; tests rely on this to stay hermetic).
- Skill `allowed-tools` and `paths` frontmatter (easy-agent stage 17 follow-ups): `allowed-tools: Bash(git commit *), Read` (CSV string or list) grants those entries as arg-scoped session allow rules when the skill is invoked — by the model via the skill tool or by the user via `/skill:name`; entries match the permission-policy rule syntax, so pre-scoped pre-approvals cover exactly the skill's needs without loosening the rest of the mode. `paths:` (gitignore-style patterns, glob values starting with `*` must be YAML-quoted) makes a skill conditional: it stays out of the model's skill listing until a read/write/edit/find call this session touches a matching path (relative to cwd; matches stick for the rest of the session), after which it joins the listing on the next prompt. Conditional skills stay invocable by name throughout.
- Grep tool fallback to grep(1) when ripgrep cannot be provisioned (offline mode, Termux, failed download): the search runs via `grep -RInE` (with `-F`/`-i`/`--include`/`-C` mapping) and returns the same `path:line: text` output shape, with a notice that `.gitignore` rules are not applied. Also exposed as `runGrepFallback`/`buildGrepFallbackArguments` for tests and remote backends.
- Teammate mail is no longer spawn-only (easy-agent stage 21): a running named teammate drains its inbox after each turn chain and, when mail arrived mid-run, wakes for another turn to act on it (turn budget still respected; mail that arrives after the budget is spent stays unread for inspection). Mailbox writes/drains and team-file member updates are serialized through a per-file keyed in-process mutex, so a `send_message` landing while a teammate drains cannot interleave read-modify-write and drop messages (easy-agent's proper-lockfile multi-writer tolerance, reduced to the in-process case).
- Full-turn auto-retry honors provider `Retry-After`: error assistant messages can carry a `retryAfterMs` hint (extracted from Retry-After/-Ms headers or SDK fields, see the pi-ai changelog); the retry waits that long instead of the backoff schedule, capped by the new `retry.maxDelayMs` setting (default 60000ms, same cap as the backoff tiers).
- `task_create` batch creation: the tool now takes a `tasks` array (each with `subject`, `description`, optional `activeForm`/`metadata`) and creates the complete task list in one call instead of one round-trip per task; ids are assigned sequentially and the result text lists every created id.

### Changed


- Upgraded Composio SDK to `@composio/core` 0.18.0 + `@composio/experimental` 0.2.3 (matching pair), silencing the SDK's runtime "Upgrade available" banner (0.17.0 reported 0.18 behind; the experimental provider base changed shape in 0.18, so both move together).
- `/memory` picker (easy-agent MemoryPicker parity): lists global MEMORY.md, project (workspace) MEMORY.md and the current session's memory file with "(new)" markers for files that don't exist yet; Enter opens the selected file in your external editor (new files get a minimal `# Memory` template), and the agent reads the file fresh on its next turn.
- Reduced-motion setting (`terminal.reducedMotion`, env `A_CODER_CLI_REDUCED_MOTION=1`, toggle in `/settings`): spinner frames render a static frame, the message shimmer sweep and in-flight tool blink dots go solid — for terminals and users who find animation noisy (easy-agent motionPrefs parity).
- Background-process output notifications are throttled to ~10Hz (leading + trailing edge) in the store itself (easy-agent bashProgressStore pattern): a chatty background command no longer drives per-chunk renders and per-chunk `background_processes_update` RPC events to the desktop; completion events still notify immediately.
- Auto-retry hardening (easy-agent stage-27 parity): a turn whose failed attempt already streamed visible output (non-empty text or thinking, or any tool call) is no longer silently replayed by the auto-retry — the error surfaces instead, avoiding duplicated visible output; and consecutive overloaded/529 failures stop auto-retrying after 3 attempts (reset by any successful turn or a non-overload retry).
- Plan-mode enrichment: entering plan mode now surfaces a persisted session plan file (`<agentDir>/plans/<sessionId>.md`) that the model can write while planning (the only write auto-approved in plan mode); obvious read-only bash commands (pwd/ls/cat/head/tail/tree/wc/stat/du/df/which/rg/grep/find/git status|diff|log|show, guarded against redirection, `&&`/`||` chaining, and destructive flags like `-delete`/`-exec`) run without an approval prompt while planning; and `plan_mode` exit accepts `plan` (content persisted to the plan file) and `allowedPrompts` (easy-agent-style pre-approved commands, e.g. `npm test`), which become arg-scoped session allow rules taking precedence over the permission mode without triggering the auto-mode classifier.
- `/rewind` now survives a session resume: file-history snapshots are mirrored into the session transcript as `file_history_snapshot` entries (invisible in LLM context; hidden in the default tree view, labeled checkpoints in "all" view), restored on load and re-hydrated when navigating session-tree branches, so rewind targets the active branch's checkpoint chain. Backup copies were already persisted on disk under the session id.
- Thinking controls: new `/think [level]` slash command (with argument completion) next to the existing keybinding cycling, and easy-agent/Claude-style thinking-keyword escalation applied at the session layer — "ultrathink" raises to the model's highest supported level, "megathink"/"think harder" to high, "think hard" to medium (raise-only, never lowers, reasoning models only); works in interactive, print, and SDK prompts.
- Persistent-memory auto-injection (opt-out via `settings.memory.enabled`): global and workspace `MEMORY.md` contents are folded into the system prompt as a `<persistent_memory>` section on prompt (re)build, capped per scope (default 2,000 chars, `settings.memory.maxCharsPerScope`). The section notes the snapshot can lag mid-session writes and to re-read via the memory tool; unreadable/missing files are skipped silently.
- Sub-agent `permissionMode` frontmatter is now enforced at spawn (`plan` enforces read-only, `auto` applies policy rules and the classifier under the normal auto path, `default` inherits the session mode as before). The before-tool-call pipeline is shared between parent and sub-agents so the override covers hooks, permission resolution, file-history tracking, and extension handlers.
- `/output-style <name>` now offers fuzzy argument completion over registered style names (the active style is filtered out).
- Background sessions (runtime registry, Phase 1 of the session architecture roadmap in `desktop-app/SESSION-ARCHITECTURE.md`): switching away from a session whose turn is still in flight no longer aborts it. `AgentSessionRuntime` now keeps a bounded registry of detached runtimes — a mid-turn runtime moved off the active slot keeps streaming to its own session file in the background and is reaped once it settles (turn ended, nothing queued); switching back re-attaches the same live runtime instead of creating a second one over the same file. When a background turn hits a tool-approval prompt or extension dialog, the runtime is flagged `needsInput` (via `markSessionNeedsInput` from the RPC host's dialog bindings) and cleared when answered, so clients can distinguish "running" from "blocked on input". New RPC: `get_sessions_status` (per-session `running`/`pendingMessageCount`/`active`/`needsInput`), `abort_session` (abort a background turn by session path), and a pushed `sessions_update` event whenever the registry changes, so clients can badge session tabs with background running/finished states. Detached runs keep their own extension bindings; `session_shutdown` fires when they are reaped. `/clear` still aborts (it rewrites the session file in place, so a detached writer would race it). Phase 3 (multi-project): the engine hosts sessions from any project — each runtime's cwd-bound services follow the session's recorded cwd — so the desktop no longer restarts the engine when switching projects. `newSession`/`new_session` accept a `cwd` (the session lands in that project's session dir); switching to a project resumes its most recent session or starts a new one. Extension-initiated replacements (`ctx.newSession()`/`ctx.switchSession()`/`ctx.fork()`) pass `discardPrevious` to preserve the stale-ctx contract (#2860): the previous runtime is disposed and the captured ctx invalidated, while user-driven switches keep it live in the background registry. Phase 2 additions: detached runtimes stay alive after their turn settles (idle keep-alive, 30-minute default reap, `setDetachedIdleTimeoutMs` override) so switching back is a pure re-attach; `session-scoped` commands — `prompt`/`steer`/`follow_up`/`abort` accept an optional `sessionPath` targeting a background session (engine resolves it via `getSessionForPath`); `switch_session` responses carry `reattached` + a `snapshot` (`running`/`needsInput`/`pendingMessageCount`) so clients can adopt in-flight turns immediately; and blocking `extension_ui_request` events (confirm/select/input/editor/question + permission prompts) are tagged with the `sessionFile` of the session that opened them, so clients can route approvals to the right session.
- `background_processes_update` RPC event: the headless/RPC mode now streams background bash-process snapshots to clients (desktop) so they can show running bash commands live, alongside the existing `subagents_update` event.
- Running-tasks viewer (Claude Code-style): press Down from an empty editor (also Shift+Down / Ctrl+Shift+B, and `/subagents`) to open a unified picker over running background bash processes and in-process sub-agents. Enter drills into a live view (bash output tail or sub-agent transcript), Esc/Left goes back, `k` kills. Sub-agent records now carry their `goal` (spawn prompt) and `model`, shown in the rows and detail view. Replaces the two separate sub-agent / background-process viewers. (a-coder-cli sub-agents are flat — they cannot spawn children — so the list is flat, not a tree.)
...
- Plan-mode "auto-accept rest of plan": when the agent requests permission for a mutating tool while plan mode is on, the prompt now offers a third option — "Yes, auto-accept rest of plan" — which approves the call and auto-approves subsequent plan-mode mutations until plan mode is disabled, so multi-step plans don't require approving every edit one-by-one. The flag resets whenever plan mode turns off (via the model's `plan_mode` tool or the user toggle).
- `/permissions` command (TUI): interactive manager for the "auto" permission-mode policy rules (allow / soft-deny / hard-deny) backed by the existing `PermissionPolicyConfig` engine. Lists every configured rule grouped by category; select a rule to remove it (with confirm) or use the "Add …" rows to add a new rule (e.g. `bash`, `edit`, `mcp__server:*`, `$defaults`), so policies can be edited without hand-editing settings.json.
...
- `ask_user_question` tool for structured multiple-choice questions (1-4 questions, headers, options, multiSelect) with a TUI `QuestionPromptComponent` and a desktop inline question card, wired over RPC via the `question` extension UI method.
...
- Composio apps gallery: a direct (non-agent) surface to browse, connect, and disconnect Composio toolkits. Adds a `/apps` command + interactive TUI selector, matching `composio_list_apps` / `composio_connect_app` / `composio_disconnect_app` RPC commands, and a desktop Apps panel (open via the command palette or `/apps` in the composer). The connect flow resolves a toolkit's auth config (reusing an existing one or creating a Composio-managed one) and calls the non-deprecated `connectedAccounts.link()` (the legacy `initiate()` retired for Composio-managed OAuth on 2026-07-03), returning a sign-in URL; the CLI awaits `waitForConnection()` to confirm, the desktop opens the link and polls by re-listing. Requires Composio to be configured (`composio.apiKey` in settings or `COMPOSIO_API_KEY`); shows a configure hint otherwise. Backed by a shared `core/composio-apps.ts` service + a `createComposioClient` factory factored out of the existing session integration so both route connections under the same stable user id.
- Per-server `suppressStderrPatterns` MCP setting (stdio servers only): regex sources tested per stderr line, whose matching lines are dropped before the server's stderr reaches the terminal. Only active when non-empty (otherwise stderr is inherited unchanged), so it opts in to silencing known-benign server noise — e.g. chrome-devtools-mcp's `No handler registered for issue code PerformanceIssue` — without hiding real errors.
- Per-server `timeoutMs` MCP setting: overrides the MCP SDK's default 60s request timeout for slow servers (browser automation, long-running queries).
- `edit` tool `edits[].replaceAll` option: replace every occurrence of `oldText` instead of failing on multi-match; non-overlapping occurrences are replaced left to right (fuzzy-matching space included).
- MCP connect timeout: the handshake is bounded to 30s (env `A_CODER_CLI_MCP_CONNECT_TIMEOUT` / `MCP_CONNECT_TIMEOUT`, in ms) so a hung server process cannot block discovery or the first tool call indefinitely.
- Settings-driven shell hooks (Claude-Code-compatible): `hooks` entries in the same settings.json files the IDE reads (`~/.a-coder|~/.claude` + project, project wins) now execute in the CLI. Wired events: `PreToolUse` (deny/approve/ask the call, JSON stdin + JSON stdout + exit-2 semantics), `PostToolUse` (flag the result as an error and/or append context), `UserPromptSubmit` (append context to the prompt; a blocking hook drops the prompt and records why in the transcript), plus fire-only `SessionStart`, `Stop`, and `PreCompact`. Matcher syntax: omitted/`*` = all, exact string, or regex when metacharacters are present; only `command`-type hooks run in the CLI (prompt/agent hooks remain IDE-only); `A_CODER_CLI_DISABLE_HOOKS=1` disables everything.
- `skill` tool: skills are now invoked by name with optional args (`$ARGUMENTS`, `${SKILL_DIR}`/legacy `${CLAUDE_SKILL_DIR}`, and `${SESSION_ID}` substitution) instead of requiring the model to locate and read SKILL.md files. `disable-model-invocation` skills refuse model invocation (user-only via `/skill:name`). The system-prompt skill listing now points at the tool.
- MCP resource tools: `mcp_list_resources` (cross-server listing, optional server filter) and `mcp_read_resource` (server + URI) expose MCP resources to the model; servers without the resources capability are skipped with a note.
- Stream-json output: `--mode stream-json` (print mode) emits a Claude-Code-flavored NDJSON envelope — a `system/init` line (session, cwd, model, tools), `assistant`/`user` message lines, passthrough `event` lines for everything else, and a final `result` line with `num_turns`, `duration_ms`, aggregated `usage`, and `total_cost_usd`.
- Arg-scoped permission rules: policy rules may scope to tool arguments — `"Bash(git push*)"`, `"write(*.env)"` — matched against the tool's primary argument (bash command, file-tool path; other tools serialize their args). In auto mode, allow now takes precedence over soft-deny so scoped allows can carve exceptions out of broad prompt rules (hard-deny still wins).
- Auto-mode LLM classifier (opt-in, `settings.autoMode.enabled`): when static rules neither allow nor deny a call in auto mode, one structured LLM call with the session's model decides whether the call looks destructive/exfiltrating/security-changing; blocks degrade to a user prompt (non-interactive keeps the static default), and repeated failures or blocks circuit-break the classifier off for the session. Read-only tools are never classified.
- Running-tasks viewer UX: the unified picker/live view over background agents and bash processes now shows per-turn token usage lines (`— turn 2 · 1,842 tok (in 1,310, out 532)`) and tool-result detail lines (`⎿ ok (1.2k chars): <first line>`) in sub-agent timelines, `… N earlier events/lines hidden` indicators above long tails, and a 1s elapsed ticker so durations advance while the viewer is open. `h` toggles hiding finished runs (easy-agent-style running-only list; the choice persists for the session, empty-state hints at the toggle).
- The `todo` tool is deprecated: removed from the default active tool set (agent sessions and SDK defaults both), with its description now pointing to `task_create`. The persistent task tools (`task_create`/`task_get`/`task_list`/`task_update`) are the default task-list mechanism; `todo` stays registered and can still be enabled explicitly via `tools`/`activeToolNames`.

### Changed

- Subagent results are no longer hard-truncated at 4,000 characters. The result returned to the parent agent, the completion event, the completion notification, and the streamed text in the `.output` log now cap at 128,000 characters (with a `... [truncated N more chars]` marker when exceeded) instead of silently slicing at 4,000/2,000. Large subagent work (long analyses, file contents) is no longer cut mid-sentence. (Centralized in `SUBAGENT_RESULT_MAX_CHARS` / `truncateSubagentResult`.)
- The inline background-processes tracker moved from above the editor to below the footer, under the permission-mode line.
...
- MCP tool calls now propagate the agent's abort signal, so cancelling a tool call in the UI cancels the in-flight MCP request instead of letting it run to the timeout. MCP tool results are rendered as model-facing text (text blocks joined, non-text blocks as placeholders, `structuredContent` fallback) instead of a raw JSON envelope. Streamable HTTP connections terminate the server-side session on close instead of just dropping the transport.
- Desktop `ask_user_question` now renders hermes-style as an interactive card inline in the transcript (at the tool-call row) instead of a modal over the chat: header chips, letter-badged option rows (A-D) with descriptions, per-question "Other" free-text row, multi-select staging, a progress count, and Skip/Answer actions with keyboard support (letter keys pick, Enter confirms). Once answered, the card settles into a read-only Q&A summary (question + answer, or "No answer" on decline) so the transcript keeps a record of what was asked. The pending request moved to the session store (`questionRequest`), keeping background-session parking/promotion behavior; the `QuestionPrompt` modal was removed.
- MCP tools are now exposed to the model as first-class agent tools: after a server connects (eagerly in the background, or on first stub use), each discovered tool is registered as `mcp__<server>__<tool>` with its real input schema and description, so the model sees exact tool names and argument shapes up front instead of discovering them through the stub's error text (which previously caused wrong-argument and invented-tool-name failures). The per-server `mcp_<server>` stub is no longer registered for servers within the 50-tool budget; it now only registers — after discovery — for aggregator servers exposing more than 50 tools, which keep stub-only to avoid flooding the tool list. Small models kept sending empty `arguments` through the stub's nested `{tool, arguments}` call shape even when correctly-shaped first-class tools existed. For those oversized servers the stub was hardened: `arguments` is optional so an empty call reaches execute, and a call resolving a tool with required properties but no arguments fails fast with the tool's input schema echoed into the error (instead of looping on the server-side validation error). `notifications/tools/list_changed` now triggers a background re-discovery so first-class tools refresh without any tool-call round-trip. Permission policy rules match with prefix patterns like `mcp__<server>__*`.

### Fixed


- Made the resource-loader test suite hermetic: the loader's cross-product skill discovery reads the real home directory (`~/.agents/skills`, `~/.a-coder/skills`, `~/.claude/skills`), so on machines with duplicate skills installed across those roots (e.g. hyperframes in both `~/.agents` and `~/.claude`) the "extendResources file URLs" test failed on strict `diagnostics == []` assertions. The suite now stubs `HOME` to the per-test temp directory (`vi.stubEnv` + `vi.unstubAllEnvs`), restoring per test.
- Background-process completion notifications no longer fire once per coexisting session: the store is process-wide, so parked runtimes, office coworker sessions and SDK sessions each saw every terminal transition and each enqueued the same `<task-notification>` note and fired its own wake turn — duplicated pills, aborted duplicate turns, and phantom API calls from disposed sessions (their store subscription was never removed). Records now carry the starting session's id; only the owning session notifies, disposed sessions unsubscribe and drop their wake loop, repeat terminal events are idempotent, and ownerless records wake nobody.
- The inline agents panel and running-tasks viewer now show sub-agent tool-use and turn counts while a run is in progress: the counters were only committed to the record at completion, so every running row read `0 tool uses` even while tools executed (last-tool/token updates already wrote through live).
- Office huddle drives now match the hermes Bot Mode semantics they were modeled on: `@all stop`/`@all resume` hold/release every coworker (previously a no-op); every user send bumps the room epoch and queues a fresh drive so a send during a long turn is never silently dropped (stale in-flight turns yield to the redirect instead of committing over it); an @mention handoff left unanswered after a quiet round gets up to two bounded continuation rounds; replies that finish after the drive ends are background-harvested (5s poll, 5-minute bound) so long work is late, never lost; Stop holds the whole room until coworkers are re-addressed ("@all resume" or a direct mention); back-to-back byte-identical duplicate replies are dropped; and a coworker's own reply counts as seen so it is not re-fed its own message next round.
...
- Fixed MCP tool failures being returned to the model as normal output: per the MCP spec, tool errors resolve as ordinary results with `isError: true` (they do not throw), and the stub tool stringified the whole envelope. Error results are now surfaced as tool errors with the server's message text, so the model sees a failure instead of JSON that looks like success.
- Fixed stale MCP connection state after a server crash or transport drop: the client now listens for transport close, marks the server down in `/mcp` and the footer chip (previously the status stayed "ok" and calls failed with raw SDK errors), drops its cached tool list, and the next tool call reconnects automatically instead of failing against the dead connection until `/reload`.
- Fixed a connect race where the eager background connect and a first tool call could each start a handshake on the same MCP client: concurrent `connect()` calls now share one in-flight handshake, and a failed handshake closes its transport so a spawned stdio process is not leaked.
- Fixed MCP tool discovery silently truncating servers that expose more tools than one page: `listTools()` now follows pagination cursors (capped at 64 pages), so large tool aggregators are listed in full.
- Fixed stale MCP tool lists mid-session: the cache is invalidated when a server sends `notifications/tools/list_changed`, and a not-found tool triggers one re-discovery before reporting failure, so server-side tool changes are picked up without `/reload`.
- Fixed the MCP client advertising a hardcoded version string ("0.80.3"); it now reports the actual package version.
- MCP stdio servers now inherit the parent environment (per-server `env` values win) instead of receiving only the SDK's narrow env allowlist — setting a per-server `env` no longer drops `PATH` and breaks `npx`-based servers. SSE transports now send custom headers on the long-lived GET event stream too, not just POSTs (authenticated servers failed on reconnect). stdio stderr is now piped with a 64 KB tail appended to connect-failure errors so a crashed server's last output isn't lost, and the full-turn auto-retry backoff now applies ±25% jitter and caps at 60s.
