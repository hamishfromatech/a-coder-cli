...
describe("openai-completions tool_choice", () => {
	...
	it("maps groq qwen3 reasoning levels to default reasoning_effort", async () => {
		const base = getModel("groq", "qwen/qwen3.6-27b")!;
		const model = { ...base, thinkingLevelMap: { minimal: null, low: null, medium: null, high: "default" } } as const;
		let payload: unknown;
	...
	it("omits tool_stream when zaiToolStream compat is disabled", async () => {
