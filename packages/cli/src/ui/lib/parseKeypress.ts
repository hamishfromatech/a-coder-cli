/**
 * Vendored from ink/build/parse-keypress.js
 * ink doesn't export this module, so we bundle our own copy.
 */
...
const kittySpecialNumberKeys: Record<string, string> = {
  2: 'insert',
  3: 'delete',
  5: 'pageup',
  6: 'pagedown',
...
const kittyCodepointNames: Record<string, string> = {
  27: 'escape',
  9: 'tab',
  127: 'delete',
  8: 'backspace',
...
const isValidCodepoint = (cp: number) =>
  cp >= 0 && cp <= 0x10_ffff && !(cp >= 0xd8_00 && cp <= 0xdf_ff);

const safeFromCodePoint = (cp: number) =>
...
function resolveEventType(value: number) {
  if (value === 3) return 'release';
  if (value === 2) return 'repeat';
  return 'press';
}

const parseKittyKeypress = (s: string) => {
...
const parseKittySpecialKey = (s: string) => {
...
export function parseKeypress(s = ''): ParsedKey {
  ...
  if (kittyResult) return kittyResult as ParsedKey;
  const kittySpecialResult = parseKittySpecialKey(s);
  if (kittySpecialResult) return kittySpecialResult as ParsedKey;
